...
When we send someone a bitcoin, what are we "really" doing? Well, we're signing a message which says that "I, keyholder of key X, am transfering title over *this* 'object' (in the example's case, a bitcoin) to the keyholder of key Y." Our signed message then is broadcst across the network. The network in turn analyzes this message to determine if the message has been properly signed, and if it has the network transfers title over the "object" in question to the keyholder of key Y.
